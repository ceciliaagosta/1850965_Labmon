<template>
  <div class="container d-flex align-items-center my-2">
    <div class="card shadow p-4" style="width: 100%; min-width: 400px;">
      <h2 class="mb-4 text-center">Edit Monster {{ props.monsterData.name }}</h2>
        <form @submit.prevent="handleSubmit">
            ...
            <button type="submit" class="btn btn-success">Edit Monster</button>
...
const form = ref({
  ...
  catch_rate: props.monsterData.catch_rate,
  collection: props.monsterData.collection,
  rarity: props.monsterData.rarity,
  sprite: props.monsterData.sprite
})
...
const handleSubmit = async () => {
  ...
  try {
    await monsterStore.updateMonster(props.monsterData.id, form.value)
    console.log(form.value)
    successMessage.value = 'Monster updated successfully!'
    form.value = {
      name: '',
      catch_rate: '',
      collection: '',
    ...
    errorMessage.value = err.response?.data?.message || 'Failed to update monster.'
  }
}
</script>
